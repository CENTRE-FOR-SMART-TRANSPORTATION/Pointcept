...
COMBINE_LABELS = ["Chevron", "broken_line",  "solid_line", "lane", "shoulder", "arrow", "marking", "lightpole","light_pole", "traffic_sign", "highway_guardrail", "vegetation", "Pavement"]
#COMBINE_LABELS = ["Chevron", "lane_marking", "All", "vegetation", "Pavement"]
...
def get_label_points(name, scene, filename_cut, orig_points, cumulative_mask, annotations, label_count, class_names):
    ...
    labels = os.path.join(
        scene, "label", f"{filename_cut}_{name}_converted.json")
    centre_file = os.path.join(
        scene, "centres", f"{filename_cut}_{name}_converted_centre.json")
    with open(labels, "r") as f:
        box_list = json.load(f)
    las = laspy.read(las_file)
    # print(las.scan_angle_rank)
...
def main():
    ...
    for scene in os.listdir(data):
        ...
        if not os.path.exists(las_files):
            print(f"No files found for {scene}")
            continue
        for filename in os.listdir(las_files):
            ...
            for label in COMBINE_LABELS:
                if os.path.exists(os.path.join(las_files, f"{filename_cut}_{label}.las")):
                    t = get_label_points(
                        label, scene, filename_cut, points_with_intensity, cumulative_mask, annotations, label_count, class_names)
