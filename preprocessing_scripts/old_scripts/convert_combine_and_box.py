...
COMBINE_LABELS = ["lane_marking", "objects_vegetation", "objects", "vegetation", "pavement"]
# FIXED_LABEL = [
#     # 'vegetation',
#     'crossbuck',
#     'stop-sign',
...
def write_points_to_txt(outfile, points):
    with open(outfile, "w") as f:
        for x, y, z, i in points:
            f.write(
                f"{round(x,20)} {round(y,20)} {round(z,20)} {(round(i,20))}\n")


def get_label_points(name, scene, filename_cut, orig_points, cumulative_mask, annotations, label_count, class_names):
...
def main():
    ...
    for scene in os.listdir(data):
        ...
        for filename in os.listdir(las_files):
            ...
            x_min, x_max = np.min(las.x)-10, np.max(las.x)+10
            y_min, y_max = np.min(las.y)-10, np.max(las.y)+10
            z_min, z_max = np.min(las.z)-10, np.max(las.z)+10
