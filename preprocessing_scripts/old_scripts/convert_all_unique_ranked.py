...
#     'sign'
#     ]
RANK_LIST = ['traffic-sign', 'delineator-post', 'wires', 'wooden-utility-pole', 'road', 'vegetation', 'clutter']
...
def remove_seen_points(points, seen, label_name):
    threshold = 1e-5
    mask = np.ones_like(points[:, 0], dtype=bool)
    for idx, t in enumerate(points):
            x, y, z, i = t
            duplicate_found = any(
                np.all(np.abs(np.array([x, y, z, i]) - np.array(p)) < threshold)
                for p in seen
            )
            if duplicate_found:
                print(f"Point for {label_name} already in seen set")
                mask[idx] = False
                continue
            seen.add((x,y,z,i))
...
def write_points_to_txt(outfile, points, seen):
    # write if not in seen set
...
def main():
    ...
    for scene in os.listdir(data):
        ...
        for filename in os.listdir(las_files):
            ...
            seen = set() # set for all the points in the file

            # group according to class name
            rank_dict = dict()
            label_points = dict()
            
            for label in RANK_LIST:
                label_points[label] = set()

            for box in box_list:
                label_name = clean_label(box["obj_type"])
                if label_name not in rank_dict:
                    rank_dict[label_name] = []
                rank_dict[label_name].append(box)
            
            # go through it according to class ranking
            for label in RANK_LIST:
                if label not in rank_dict:
                    continue
                rank_box_list = rank_dict[label]
    
                for box in rank_box_list:
                    psr = box["psr"]
                    position = np.array(
                    ...
                    if label_name == "vegetation":
                        print(f"Downsampling {label_name}", selected_points.shape)
                        selected_points = voxel_downsample(selected_points, 0.5)
                        print(f"Size after downsampling", selected_points.shape)                
                    if label_name == "road":
                        print(f"Downsampling {label_name}", selected_points.shape)
                    ...
                    # seen_mask = remove_seen_points(info.T, seen, label_name)
                    # count = sum([1 if x else 0 for x in seen_mask])
                    # print("count", count)
                    # points_arr = points_arr[seen_mask]
                    # intensity_arr = intensity_arr[seen_mask]
                    points_arr = pc_normalize(points_arr.T)
                    intensity_arr = intensity_normalize(intensity_arr)

                    info = np.vstack(
                        (points_arr[:, 0], points_arr[:, 1], points_arr[:, 2], intensity_arr))
                    print(f"Writing points for {label_name} to {outfile}...")
                    write_points_to_txt(outfile, info.T, seen)
            ...
            if DOWNSAMPLE_CLUTTER:
                ...
                unlabeled_points = voxel_downsample(unlabeled_points, 0.5)
