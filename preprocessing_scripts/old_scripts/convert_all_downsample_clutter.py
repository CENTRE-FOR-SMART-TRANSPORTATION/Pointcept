...
def intensity_normalize(intensity_arr):
    centroid = np.mean(intensity_arr)
    intensity_arr = intensity_arr - centroid
    ...
    for scene in os.listdir(data):
        ...
        for filename in os.listdir(las_files):
            ...
            for box in box_list:
                ...
                print(info.shape, points_arr.shape, intensity_arr.shape, points_with_intensity.shape)
            ...
            print(f"Clutter shape {unlabeled_points.shape}")
            unlabeled_points = voxel_downsample(unlabeled_points, VOXEL_SIZE)
            print(f"shape after downsample {unlabeled_points.shape}")
            points_arr = np.vstack((unlabeled_points[:, 0], unlabeled_points[:, 1], unlabeled_points[:, 2]))
            intensity_arr = unlabeled_points[:, 3]
            points_arr = pc_normalize(points_arr)
            intensity_arr = intensity_normalize(intensity_arr)
            print(intensity_arr.shape, points_arr.shape)
            info = np.vstack((points_arr[:, 0], points_arr[:, 1], points_arr[:, 2], intensity_arr))
