...
COMBINE_LABELS = ["lane_marking", "all", "objects_vegetation", "objects", "vegetation", "pavement"]
# FIXED_LABEL = [
#     # 'vegetation',
#     'crossbuck',
#     'stop-sign',
...
#     'sign'
#     ]
FIXED_LABEL = ['traffic-sign', 'solid-line', 'broken-line',
               'wires', 'wooden-utility-pole', 'road', 'vegetation', 'clutter']
...
def get_label_points(name, scene, filename_cut, orig_points, cumulative_mask, annotations, label_count, class_names, pc_min, pc_m, intensity_min, intensity_m):
    ...
    points_with_intensity = np.vstack((las.x, las.y, las.z, las.intensity)).T
    ...
    outer_seen = set()
    for x, y, z, i in np.round(orig_points[cumulative_mask], 6):
        outer_seen.add((x, y, z, i))
    
    label_mask = np.zeros_like(points[0, :], dtype=bool)

    for idx, t in enumerate(np.round(points_with_intensity, 6)):
        x, y, z, i = t
        if (x, y, z, i) in outer_seen:
            label_mask[idx] = True
        
    for x, y, z, i in np.round(points_with_intensity, 6):
        seen.add((x, y, z, i))
    ...
    for box in box_list:
        ...
        selected_points = voxel_downsample(selected_points, VOXEL_SIZE)
        print(f"Size after downsampling", selected_points.shape)
        # elif label_name == "road":
        #     print(f"Downsampling {label_name}", selected_points.shape)
        #     selected_points = voxel_downsample(selected_points, 0.08)
        #     print(f"Size after downsampling", selected_points.shape)
    ...
    if len(unlabeled_points) > 0:
        ...
        selected_points = voxel_downsample(unlabeled_points, VOXEL_SIZE)
...
def main():
    ...
    for scene in os.listdir(data):
        ...
        for filename in os.listdir(las_files):
            ...
            downsampled_whole_file = voxel_downsample(points, VOXEL_SIZE)
            points_arr = np.vstack(
                (downsampled_whole_file[:, 0], downsampled_whole_file[:, 1], downsampled_whole_file[:, 2]))
            intensity_arr = downsampled_whole_file[:, 3]
            points_arr = pc_normalize(points_arr, pc_min, pc_m)
            intensity_arr = intensity_normalize(intensity_arr, intensity_min, intensity_m)

            info = np.vstack(
                (points_arr[:, 0], points_arr[:, 1], points_arr[:, 2], intensity_arr))
            print(f"Writing points for check to chec.txt...")
            write_points_to_txt(os.path.join(outfolder, "check.txt"), info.T)
            print("wrote poitns to outfile")
            ...
            for label in COMBINE_LABELS:
                if os.path.exists(os.path.join(las_files, f"{filename_cut}_{label}.las")):
                    t = get_label_points(
                        label, scene, filename_cut, points_with_intensity, cumulative_mask, annotations, label_count, class_names, pc_min, pc_m, intensity_min, intensity_m)
